...
export async function getUserTokens(userId: string) {
  const sql_client = getSql()
  const result = await sql_client("SELECT * FROM user_tokens WHERE user_id = $1", [userId])
...
export async function getUserProperties(userId: string) {
  const sql_client = getSql()
  return await sql_client("SELECT * FROM gsc_properties WHERE user_id = $1 ORDER BY created_at DESC", [userId])
}
